...
public class zjxt_topo {
	...
	public void IniLinks() throws Exception {
		...
		for (Entry<String, zNode> entry : zNodeList.entrySet()) {
			tmp = entry.getValue();
			if (tmp.LinkInfo == null || "".equals(tmp.LinkInfo.trim())) {
			...
			for (int j = 0; j < info.length; j++) {
				...
				if (tmp2 == null)
					throw new Exception("IniLinks()->拓扑信息异常:找不到ID为" + info[j]
							+ "的节点!");
				tmp.Links.add(tmp2);
			}
		}
	...
	public void replaceTnode() {
		Iterator<Entry<String, zNode>> iterator = zNodeList.entrySet()
				.iterator();
	...
	public boolean LoadFromDb() {
		try {
			...
			for (Entry<String, zNode> entry : zNodeList.entrySet()) {
				zjxt_msg.show("id=" + entry.getKey() + ", parent="
						+ entry.getValue().parentId);
			}
			for (Entry<String, zNode> entry : zNodeList.entrySet()) {
				String info = "";
	 ...
	 * 
	 * @Title: getBeginNode
	 * @Description: 获取首端节点 有可能多条馈线在一张拓扑图，所以会存在多个馈线首端节点
	 * @author: lixu
	 * @return
	 */
	...
	public String GetResultInfo() {
		...
		for (int i = 0; i < ct; i++) {
			tmp = Result.get(i);
			sBuilder.append("(Name:").append(tmp.Name).append(" Id:")
					.append(tmp.Id).append(" Style:").append(tmp.Style)
					.append(')');
		}
	...
	public static class zNode {
		...
		public List<zNode> getChildzNodes() {
			List<zNode> nodes = new ArrayList<zNode>();
			for (Entry<String, zNode> entry : zjxt_Initialize.topo.zNodeList
					.entrySet()) {
